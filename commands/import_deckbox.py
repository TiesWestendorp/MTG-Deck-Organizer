...
def import_deckbox(user_id, folder):
    user_url = "https://deckbox.org/users/{}"
    export_url = "https://deckbox.org/sets/{}/export"
    ...
    def download_deck(deck_id):
        ...
        response = requests.get(export_url.format(deck_id))
        if not response.ok:
            raise response.reason
        parser = DeckParser()
        parser.feed(response.content.decode("utf-8"))
...
if __name__ == "__main__":
    ...
    import_deckbox(deckbox_user_id, deckbox_folder)
